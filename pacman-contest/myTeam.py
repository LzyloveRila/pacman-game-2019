...
class PositionSearchProblem:
    ...
    def __init__(self, gameState, agent, agentIndex = 0,costFn = lambda x: 1):
        ...
        self.lastEatenFood = agent.FoodlastAtePos

        self.food = agent.getFood(gameState).asList()

        if (len(agent.oldfood) < len(self.food)):

            extrasafefood = agent.foodClass.getSafeFoods(gameState, agent.oldfood,True)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if self.ghostsPos and self.suicide == False:
...
class Defender(BasicAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if len(pacmans) == 0 or gameState.getAgentPosition(self.index) == self.FoodlastAtePos or len(nearpacmans) > 0:
            self.FoodlastAtePos = None
        ...
        if  len(pacmans) > 0 and myState.scaredTimer == 0:
            ...
            if len(nearpacmans)==0 and self.FoodlastAtePos!=None:
                problem = PacmanLastPosition(gameState, self, self.index)
                self.problem == "PacmanLastPosition"
...
class AstarAttacker(BasicAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if actions != None and len(actions) != 0:
            ...
            if currPos not in self._boundary(gameState):
                self.suicide = True
                problem = goHome(gameState, self, self.index)
                actions = self.aStarSearch(problem, gameState, self.nullHeuristic)
                self.suicide = False
                if actions != None and len(actions) != 0:
                    return actions[0]
...
class PacmanLastPosition(PositionSearchProblem):
...
class DistinctFoodType:
    ...
    def getSafeFoods(self, gameState,oldfood=None,update = False):
        ...
        for food in foodlist:
            ...
            x = food[0]
            y = food[1]
            f_sides.append((x + 1, y))  # right
            f_sides.append((x - 1, y))  # left
            f_sides.append((x, y + 1))  # up
            f_sides.append((x, y - 1))  # down
            for f_fr in f_sides:
    ...
    def NumOfPath(self, foods):
        ...
        path = 0
        for side in f_point:
            close_list = copy.deepcopy(visited)
            if self.BFS(side, close_list):
                path = path + 1
        return path

    def BFS(self, f_point, closed,goal = None):

        side = util.Queue()
        side.push((f_point, []))
        while not side.isEmpty():
            state, actions = side.pop()
            closed.append(state)
            ...
            for successor, direction in self.getSuccessors(state):
                if successor not in closed:
                    ...
                    side.push((successor, actions + [direction]))
